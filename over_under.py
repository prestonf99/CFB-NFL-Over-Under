...
def get_over_under():
    data = nfl.import_schedules(range(1999, 2025))
    data = data[data['game_type'].isin(['REG'])]
    ...
    raw = data[['gameday', 'season', 'week', 'home_team', 'away_team', 'team_favored',
                'spread_favorite', 'temp', 'wind', 'home_score', 'away_score', 'total_score', 'total_line',
                'over_under_result', 'result', 'home_favorite', 'favorite_covered',
                'winning_team', 'losing_team', 'home_wins', 'home_losses', 'away_wins', 'away_losses',
                'home_points_for','home_points_against', 'away_points_for', 'away_points_against', 
                'home_win_pct','away_win_pct', 'win_pct_diff', 'h_ppg', 'h_papg', 'a_ppg', 'a_papg',
                'home_pt_diff_pg', 'pt_diff_pg', 'away_pt_diff_pg', 'home_win_pct_last_4',
               'away_win_pct_last_4', 'home_win_prob', 'away_win_prob']].copy()
    raw['temp'].fillna(70, inplace=True)
